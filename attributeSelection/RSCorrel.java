...
public class RSCorrel extends ASEvaluation implements SubsetEvaluator, OptionHandler, TechnicalInformationHandler { 
	...
	public Measure m_RelevancyMeasure= new WeakGamma();
	...
	public void setOptions (String[] options) throws Exception {
		...
		if(optionString.length() != 0) {
			...
			setRelevancyMeasure( (Measure) Utils.forName( Measure.class, className, moreOptions) );
		}
		else {
			setRelevancyMeasure(new WeakGamma());
		}
		...
		if(optionString.length() != 0) {
			...
			setRedundancyMeasure( (UnsupervisedFuzzyMeasure) Utils.forName( Measure.class, className, moreOptions) );
		}
		else {
	...
	public void setRelevancyMeasure(Measure fe) {
		m_RelevancyMeasure = fe;
	}

	public Measure getRelevancyMeasure() {
		return m_RelevancyMeasure;
	}
	...
	public String[] getOptions ()  {
		...
		result.add("-Z");
		result.add((m_RelevancyMeasure.getClass().getName() + " " +
				Utils.joinOptions(m_RelevancyMeasure.getOptions())).trim());

		result.add("-Y");
		result.add((m_RedundancyMeasure.getClass().getName() + " " +
	...
	public String toString () {
		...
		if (m_trainInstances == null) {
			...
			text.append("\nFuzzy rough correlation\n\nMethod: "+m_RelevancyMeasure);
			if (m_RelevancyMeasure instanceof IVFuzzyMeasure) {

				text.append("\nParameter = "+((IVFuzzyMeasure)m_RelevancyMeasure).getParameter()+"\n");
			}
	...
	throws Exception {
		...
		m_dep_matrix = new float [m_numAttribs][m_numAttribs];

		for (int i = 0; i < m_dep_matrix.length; i++) {
			m_dep_matrix[i][i] = 1.0f;
			m_std_devs[i] = 1.0;
		}

		for (int i = 0; i < m_numAttribs; i++) {
			for (int j = 0; j < m_numAttribs; j++) {
				m_dep_matrix[i][j] = -999;
			}
		}
		...
		m_RelevancyMeasure.set(m_Similarity,m_DecisionSimilarity,m_TNorm,m_composition,m_Implicator,m_SNorm,m_numInstances,m_numAttribs,m_classIndex,m_trainInstances);
		m_RedundancyMeasure.set(m_Similarity,m_DecisionSimilarity,m_TNorm,m_composition,m_Implicator,m_SNorm,m_numInstances,m_numAttribs,m_classIndex,m_trainInstances);
	...
	public double evaluateSubset (BitSet subset) throws Exception {
		...
		BitSet subs = new BitSet(m_numAttribs);

		if (subset.cardinality()>0) {
			try {

				// do numerator
				//num = m_Measure.calculate(subset);
				
				for (int i = subset.nextSetBit(0); i >= 0; i = subset.nextSetBit(i+1)) {
					if (i != m_classIndex) {
						if (m_dep_matrix[i][m_classIndex] == -999) {
							subs.set(i);
							float corr = (float)m_RelevancyMeasure.calculate(subs);
							subs.clear(i);
							m_dep_matrix[i][m_classIndex] = corr;
							m_dep_matrix[m_classIndex][i] = corr;
								
						}
						num += m_dep_matrix[i][m_classIndex];//+m_dep_matrix[m_classIndex][i];
						
						//alt denominator	
						/*
						for (int j = subset.nextSetBit(0); j >= 0; j = subset.nextSetBit(j+1)) {  
						 
						subs.set(i);subs.set(j);
						
						double joint = m_Measure.calculate(subs);
						subs.clear(i);subs.clear(j);
						denom += joint - m_dep_matrix[i][j] - m_dep_matrix[j][i];
					...
					*/
						...
						//denominator
						for (int j = subset.nextSetBit(0); j >= 0; j = subset.nextSetBit(j+1)) {
							if (j!=m_classIndex&&j!=i) {
								if (m_dep_matrix[i][j] == -999) {
									subs.set(i);
									double corr1 = m_RedundancyMeasure.calculate(subs, j);
									subs.clear(i);
									m_dep_matrix[i][j] = (float)corr1;
								}
								if (m_dep_matrix[j][i] == -999) {
									subs.set(j);
									double corr2 = m_RedundancyMeasure.calculate(subs, i);
									subs.clear(j);
									m_dep_matrix[j][i] = (float)corr2;
								}
								
								//page 15: http://www2.cs.uregina.ca/~yyao/PAPERS/information_measures.pdf
								denom += (m_dep_matrix[i][j]+m_dep_matrix[j][i])/2;
							}
						}
				...
				double card = subset.cardinality();
				
				//the mRMR approach - doesn't work here
				//eval = (1/card)*num - ((1/Math.pow(card, 2))*denom);
				
				//The Correlation FS approach
				eval = (num)/Math.sqrt(card + (card-1)*denom);
	...
	public static void main (String[] args) {
		runEvaluator(new RSCorrel(), args);
	}
}
