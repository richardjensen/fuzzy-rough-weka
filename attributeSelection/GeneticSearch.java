...
implements StartSetHandler, OptionHandler, TechnicalInformationHandler {
	...
	implements Cloneable, Serializable, RevisionHandler {
		...
		public String getRevision() {
			return RevisionUtils.extract("$Revision: 6761 $");
		}
	}
   ...
   <!-- options-end -->
	 ...
	 */
	private void initPopulation(BitSet core) throws Exception {
		int i,j,bit;
		int num_bits;
		boolean ok;
		int start = 0;
		...
		for (i=start;i<m_popSize;i++) {
			...
			//seed the initial population with the core if it has been calculated
			if (core!=null) m_population[i].getChromosome().or(core);
		}
	}
