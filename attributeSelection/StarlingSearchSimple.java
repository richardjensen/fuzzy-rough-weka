...
import weka.attributeSelection.StarlingSearch.Particle;
...
/** 
 <!-- globalinfo-start -->
 * StarlingSearchSimple:<br/>
 * <br/>
 * Performs a search using Starling Optimisation.<br/>
 * <br/>
 *
 * 
 *
 * @author Boidy ap Boids
 */
public class StarlingSearchSimple
extends ASSearch 
implements OptionHandler, TechnicalInformationHandler {
	...
	/** alignment constant - bird model **/
	private double m_delta = 3;
	...
	/** number of birds to consider (kNN) */
	private int m_kNN = 7;
	
	/** whether neighbours are generated randomly */
	private boolean NN_flag = false;
	...
	/** random variable for experimenter */
	public int dummyVar=1;
	
	// Inner class
	/**
	 * A bird for the starling algorithm
	 */
	...
	public Enumeration<Option> listOptions () {
		...
		newVector.addElement(new Option("\tSet the value of beta."
				+"\n\t(default = 2)" 
				, "B", 1, "-B <beta value>"));
		newVector.addElement(new Option("\tSet kNN."
				+"\n\t(default = 7)" 
				, "N", 1, "-N <NN>"));
	...
	throws Exception {
		...
		if (optionString.length() != 0) {
			setDelta((new Double(optionString)).doubleValue());
		}
		...
		setNNSearch(Utils.getFlag('N', options));
	...
	/**
	 * Gets the current settings of StarlingSearchSimple.
	 *
	...
	public String[] getOptions () {
		...
		String[] options = new String[19 + heuristicOptions.length+1];
		int current = 0;
	...
	public String NNSearchTipText(){
		return "Find neighbouring starlings using distance metric";
	}
	
	public void setNNSearch(boolean n){
		NN_flag = n;
	}
	
	public boolean getNNSearch(){
		return NN_flag;
	}
	...
	/**
	 * get the bird model constant delta
	 * @return delta, the bird model constant
	 */
	...
	/**
	 * set the bird model constant delta
	 * @param c, bird model constant
	 */
	public void setDelta(double c) {
		m_delta = c;
	}
	
	/**
	 * get the k-nearest-neighbours
	 * @return k, the number of nearest neighbours
	 */
	public int getkNN(){
		return m_kNN;
	}
	 ...
	 * @param l, the number of nearest neighbours
	 */
	public void setkNN (int l){
		m_kNN = l;
	}
	...
	public String globalInfo() {
		return 
		"StarlingSearchSimple:\n\nPerforms a search using binary Flock of Starlings Optimization.\n\n"
		+ "A number of birds are initialized at random locations (which correspond to feature subsets) and then flock towards promising areas via the global"
		+ " best solution so far and each birds's local best. "
	...
	/**
	 * Constructor. Make a new StarlingSearchSimple object
	 */
	public StarlingSearchSimple() {
		resetOptions();
		rand = new Random();
		dummyVar = rand.nextInt();
	...
	public String toString() {
		...
		desc.append("\n\tDelta: "
				+Utils.doubleToString(m_delta,6,3));
	...
	/**
	 * Searches the attribute subset space using a Starling algorithm.
	 *
	...
	throws Exception {
		...
		for (int i=1;i<=m_maxGenerations;i++) {
			...
			for (int p = 0; p < m_numParticles; p++) {
				...
				Neighbour [] nearestNeighbours  = calculateNeighbours(m_population[p], m_kNN);
				
				// get the sum of distances of all particles from p
				int sumOfVels = 0; 
				for(int m=0;m<nearestNeighbours.length; m++) {
					int indx = nearestNeighbours[m].getIndex();
					sumOfVels += Math.abs(m_population[indx].velocity);
				}
				System.out.println("sumOfVels: " + sumOfVels);
				double mccb = 1/m_kNN * sumOfVels;
				
				
				updateParticle(m_population[p], mccb);			
			 }
			}
			//else just choose random neighbours
			else if (!NN_flag){
				for (int p = 0; p < m_numParticles; p++) {
					//Calc neighbours
				
				int [] randNN = calculateRandomNeighbours(m_population[p], m_kNN);
				
				int sumOfVels = 0;
				for(int m=0;m<randNN.length; m++) {
					int indx = randNN[m];
					sumOfVels += Math.abs(m_population[indx].velocity);
				}
				System.out.println("sumOfVels: " + sumOfVels);
	...
	private int[] calculateRandomNeighbours (Particle p, int rknn){
		
		int [] list = new int[rknn];
		
		
		for (int j=0; j<rknn; j++){
			
			list [j] = m_random.nextInt(m_numParticles);
	...
	private void updateParticle(Particle p, double mccb) {
		int distance=0;
		int gdistance=0;
		...
		int vel = (int)(m_Weight*p.velocity) + (int)(m_c1*m_random.nextDouble()*distance) + (int)(m_c2*m_random.nextDouble()*gdistance) + (int)(m_delta*m_random.nextDouble()*mccb);
