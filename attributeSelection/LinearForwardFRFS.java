 ...
 */
public class LinearForwardFRFS 
extends ASSearch 
implements RankedOutputSearch, StartSetHandler, OptionHandler {
	...
	public String globalInfo() {
		return "LinearForwardFRFS - select only those features with gamma > 0";
	}
   ...
   <!-- options-end -->
	...
	public String toString() {
		StringBuffer FString = new StringBuffer();
		FString.append("\tLinear forward search for FRFS.\n\tStart set: ");
	...
	throws Exception {
		...
		if (best_merit==1 && (m_ASEval instanceof FuzzyRoughSubsetEval)) {
		...
		for (i=0;i<m_numAttribs;i++) {
			if ((i != m_classIndex) && (!temp_group.get(i))) {
				...
				if (temp_merit>alpha) m_best_group.set(i);
