...
public class FuzzyMutInf extends FuzzyMeasure implements TechnicalInformationHandler  {
	double denom;
				...
				for (int j = i+1;j<m_numAttribs;j++) {
					if (subset.get(j)&&j!=m_classIndex) {
						...
						if (correl[i][j]==NOT_CALCULATED) {
							...
							//if (correl[i][j]<0) System.err.println(i+ " "+ j +" djdiofodhofshf");
						}
	...
	public double calculate(int a1, int a2) {
		...
		for (int i=0;i<m_numInstances;i++) {
			...
			for (int j=0;j<m_numInstances;j++) {
				...
				//				sum of these
				// should be Min here, and not general Tnorm
				tnorm += Math.min(sim1, sim2);
				//tnorm += m_TNorm.calculate(sim1, sim2);

			}	

			/*h1 += log2(sum1/numInstances);
			h2 += log2(sum2/numInstances);
			joint +=log2(tnorm/numInstances);*/
			
			ret+=log2((sum1*sum2)/(numInstances*tnorm));
		}

		ret = -ret/numInstances;
		
		/*h1 = - h1/numInstances;
		h2 = - h2/numInstances;
		joint = - joint/numInstances;
		ret = h1 + h2 - joint;*/

		//if (ret<0) System.err.println(ret+ ":-  "+h1+" + "+h2+" - "+joint);

		return ret;
	}
	
	
	//calculate symmetric uncertainty between two attributes
	public double symmetricUncertainty(int a1, int a2) {
		double ret=0;
	...
	public String globalInfo() {
		return "Fuzzy mutual information-based evaluation.\n\n"
		+ getTechnicalInformation().toString();
	}

	public String toString() {
		return "Fuzzy mutual information";
	}
