...
public class GammaOWAQ extends FuzzyMeasure  implements TechnicalInformationHandler {
			...
			if (indexes[x] != 0) {
				...
				for (int d = 0; d < m_numInstances; d++) {
					if (decIndexes[d] != 0) {
						...
						// lower approximations of object x: here: minimum, later: change in the new class to a min-like owa 
						...
						for(Double implication: implications){							
							sum = sum + implication*weight(total-i,alpha,beta,total);
							i++;
						}
	...
	public String globalInfo() {
		return "Gamma evaluator, using the OWA operator for the lower approximation.\n\n"
		+  getTechnicalInformation().toString();
	}

	public String toString() {
		return "GammaOWA"+" alpha: "+ alpha + " beta: "+ beta;
	}
