...
implements UnsupervisedFilter, OptionHandler, Serializable {
	...
	protected boolean m_UseMissing = true;
	...
	public String globalInfo() {
		return "Adds noise to the decision feature (supports both nominal and numeric values). For numeric values, the noise is Gaussian.";
	}
	...
	public boolean batchFinished() throws Exception {
		...
		if (!isFirstBatchDone()) {
			...
			if (m_trainInstances.attribute(m_classIndex).isNumeric()) {
				stddev1 = (m_trainInstances.attributeStats(m_classIndex)).numericStats.stdDev;
				max1 = (m_trainInstances.attributeStats(m_classIndex)).numericStats.max;
				min1 = (m_trainInstances.attributeStats(m_classIndex)).numericStats.min;
			}

			setOutputFormat(m_trainInstances);

			Random random = new Random(m_RandomSeed);
			int iterations = (int)(m_noiseProportion*m_numInstances);

			m_trainInstances.randomize(random);

			for (int i=0;i<iterations;i++) {
				corrupt(m_trainInstances,i,m_classIndex,random);
			}
	...
	public static void main(String [] argv) {
		runFilter(new AddClassNoise(), argv);
	}
}
