...
/*
 *    Copyright (C) 2013 Richard Jensen
 *
 */
 ...
 */
public class MinorityObjectGenerator 
extends Filter
implements SupervisedFilter, OptionHandler, Serializable {
	...
	public String globalInfo() {
		return "Balances the dataset by using a classifier to classify constrained randomly generated instances";
	}
	...
	public boolean batchFinished() throws Exception {
		...
		if (!isFirstBatchDone()) {
			...
			//need to loop and add instances accordingly
			...
			for (int c=0;c<numberOfClasses;c++) {
				...
				for (int i=0;i<toCreate[c];i++) {
					...
					//reduce the threshold if we're getting nowhere
					if (problems>20) {
						threshold*=0.9;
					}
				}
			...
			/*while(totalToCreate > 0) { //while there are still instances to create
				//generate a random instance
				Instance newInstance = generateRandomInstance(m_trainInstances,random);
				//classify the instance
				double[] dist = m_classifier.distributionForInstance(newInstance);

				//if the instance belongs maximally (or over threshold) to a particular class
				//then check to see if that class has instances to be created and update
				for (int c=0;c<numberOfClasses;c++) {
					if (dist[c]>=m_threshold) {
						if (toCreate[c]>0) {
							newInstance.setClassValue(c);
							toAdd.add(newInstance);
							toCreate[c]--; totalToCreate--;
						}
						break;
					}
				}				
	...
	public static void main(String [] argv) {
		runFilter(new MinorityObjectGenerator(), argv);
	}
}
