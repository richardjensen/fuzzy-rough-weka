...
implements SupervisedFilter, OptionHandler, Serializable, AdditionalMeasureProducer {
	...
	public boolean batchFinished() throws Exception {
		...
		if (!isFirstBatchDone()) {
			...
			double[] sums = new double[m_numClasses]; // used for the probabilistic selection of instances
			for (int d = 0; d < classCounts.length; d++) sums[d]=0;
			...
			for (int d=0; d < m_numClasses; d++) {
				...
				// generate the new instances: probabilistically choose two instances based on the 
				// lower approx (or other measure) and then generate an instance between these, weighted by the lower approx (or other measure) 
				for (int i=0; i<numToGenerate; i++) {
					double probability = sums[d]*rand.nextDouble();
					Index ins1 = getChosenInstance(probability,perDecision[d],-1);
					probability = sums[d]*rand.nextDouble();
					Index ins2 = getChosenInstance(probability,perDecision[d],ins1.object);

					Instance instance1 = m_trainInstances.get(ins1.object);
					Instance instance2 = m_trainInstances.get(ins2.object);
					double ins1Weight = ins1.measure/(ins1.measure+ins2.measure);
					double ins2Weight = ins2.measure/(ins1.measure+ins2.measure);
					...
					while(attrEnum.hasMoreElements()) {
						...
						if (!attr.equals(getInputFormat().classAttribute())) {
							if (attr.isNumeric()) {
								// numeric: weighted average of the two values based on the lower approximation
								values[attr.index()] = ins1Weight*instance1.value(attr) + ins2Weight*instance2.value(attr);
							} else {							
								// nominal: choose the value of the instance that belongs the most to the lower approximation
								int i1value = (int) instance1.value(attr);
								int i2value = (int) instance2.value(attr);

								if (ins1Weight>=ins2Weight) values[attr.index()] = i1value;
								else values[attr.index()] = i2value;
							}
						}
					}
	...
	/**
	 * Probabilistically choose an instance from the ordered 'list' based on lower approximation 
	 * memberships (or other measure)
	 * @param probability the generated probability
	 * @param list sorted list of instance indexes based on the measure of those instances
	 * @param prevInstance the index of the previously selected instance, used to avoid choosing the same instance again 
	 * @return the index of the chosen instance
	 */
	private Index getChosenInstance(double probability, ArrayList<Index> list, int prevInstance) {
		double sumSoFar=0;

		for (int i=0; i<list.size();i++) {
			Index next = list.get(i);
			sumSoFar += next.measure;
			if (sumSoFar>=probability) {
				if (next.object==prevInstance) continue; // move on to the next element
				else return next;
			}
		}
	...
	public static void main(String [] argv) {
		runFilter(new FROversample(), argv);
	}
