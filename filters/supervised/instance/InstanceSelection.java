		...
		if (!isFirstBatchDone()) {
			...
			while(gamma!=1) {
				...
				for (int i=0;i<m_numInstances;i++) {
					copy = new Instances(m_trainInstances);
					copy.delete(i);
					
					m_Similarity.setInstances(copy);
					m_DecisionSimilarity.setInstances(copy);
					m_SimilarityEq.setInstances(copy);
					m_composition = m_Similarity.getTNorm();

					m_Measure.set(m_Similarity,m_DecisionSimilarity,m_TNorm,m_composition,m_Implicator,m_SNorm,m_numInstances-1,m_numAttribs,m_classIndex,copy);

					double tempGamma = m_Measure.calculate(full);
				...
				if (removeObjectIndex>-1) {
					getInputFormat().delete(removeObjectIndex);
					count++;
					System.err.println(removeObjectIndex+" => "+best+"  ("+getInputFormat().numInstances()+")");
					gamma=best;
			...
			removed=count;
			System.err.println("Removed "+count+" instance(s)");
