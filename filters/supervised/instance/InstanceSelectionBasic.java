/** 
For FRIS-I, use InstanceSelectionBasic with default settings; 
For FRIS-II, use InstanceSelectionBasic and set the ‘iterative’ flag to 'true'
*/
 ...
 */
public class InstanceSelectionBasic 
extends Filter
implements SupervisedFilter, OptionHandler, Serializable, AdditionalMeasureProducer {
		...
		if (!isFirstBatchDone()) {
			if (!iterative) {
				...
				for (int i = 0; i < getInputFormat().numInstances(); i++) {
					...
					if (mems[i]>=m_threshold) push(getInputFormat().instance(i));
					else count++;
				}

				removed=count;
				System.err.println("Removed "+count+" instances");
				...
				while(go) {
					m_trainInstances = getInputFormat();
					...
					for (int i = 0; i< mems.length; i++) {
						if (mems[i]<m_threshold && mems[i]<lowestValue) {
							lowestValue = mems[i];
							lowestIndex = i;
						}
					...
					if (lowestIndex>-1) {
						//remove the object (indexed by lowestIndex) and loop again
						getInputFormat().delete(lowestIndex);
						
						count++;
					}
					else go = false; //all objects are consistent so the loop is terminated here
					
					
				}
				
				/*BitSet reduct = new BitSet(getInputFormat().numAttributes());
				reduct.set(1);
				reduct.set(4);
				*/
				...
				for (int i = 0; i < getInputFormat().numInstances(); i++) {
					...
					//for (int a=0;a<reduct.cardinality();a++) if (reduct.get(a)) temp.deleteAttributeAt(a);
	...
	public static void main(String [] argv) {
		runFilter(new InstanceSelectionBasic(), argv);
	}
	...
	//number of instances that have been removed
	public double removed = 0;
