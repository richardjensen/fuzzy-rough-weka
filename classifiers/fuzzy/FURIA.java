...
TechnicalInformationHandler {    
   ...
   <!-- options-end -->
	 ...
	 * @return the ruleset
	 */
	public ArrayList<Rule> getRuleset(){ return m_Ruleset; }
	...
	public RuleStats getRuleStats(int pos) {
		return (RuleStats)m_RulesetStats.get(pos);
	}
	...
	public void buildClassifier(Instances instances) throws Exception {  
		...
		for(int z=0; z < m_RulesetStats.size(); z++){
			RuleStats oneClass = (RuleStats)m_RulesetStats.get(z);
	...
	public double[] distributionForInstance(Instance datum) throws Exception{ 
		...
		if (Utils.sum(rulesCoveringForEachClass)==0){
			...
			ArrayList<Rule> origRuleset = (ArrayList<Rule>) m_Ruleset.clone();
	...
	protected Instances rulesetForOneClass(double expFPRate, 
			...
			oneRule:    
				while(!stop && hasPositive){			
					...
					if(isResidual){
						...
						ArrayList tempRules = (ArrayList)ruleset.clone();
		...
		m_Ruleset.addAll(ruleset); //appendElements
	...
	public String toString() {
		...
		for(int j=0; j<m_RulesetStats.size(); j++){
			RuleStats rs = (RuleStats)m_RulesetStats.get(j);
