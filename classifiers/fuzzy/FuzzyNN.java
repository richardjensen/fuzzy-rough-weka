...
implements OptionHandler, TechnicalInformationHandler {
 ...
 <!-- options-end -->
	...
	public String toString() {
		...
		String result = "FuzzyNN (using normalized Euclidean distance)\n" +
				"using " + m_kNN;
	...
	protected void init() {
		...
		/*for(int index=0; index<m_Train.numAttributes(); index++) {
			if (index != m_Train.classIndex()) {
				AttributeStats as = m_Train.attributeStats(index);
			}
		}*/
	}
	...
	public Instances kNearestNeighbours(Instance instance, int m_kNN) {
		...
		for(int o=0; o<m_Train.numInstances(); o++) {
				...
				if (i != m_Train.classIndex()) {
					...
					if(Double.isNaN(mainVal)||Double.isNaN(otherVal))
						...
						if(m_Train.attribute(i).isNominal()){
							...
							double minVal = m_Similarity.min[i];
							double maxVal = m_Similarity.max[i]; 
							double noemer;
							if(minVal==maxVal)
								noemer = 1;
							else
								noemer=maxVal-minVal;

							double mainValnorm = (double) (mainVal-minVal)/(noemer);
							double otherValnorm = (double) (otherVal-minVal)/(noemer);

							dist +=(mainValnorm-otherValnorm)*(mainValnorm-otherValnorm);
						}
					}
				}
			}
			...
			if(dist==0.0)
				...
				sim =(double) 1/ Math.pow(dist, (double) 1 / (m_fuzzifier - 1));

			objectIndex.add(new Index(o, dist));
			m_Train.instance(o).setWeight(sim);
		}
		Collections.sort(objectIndex,Collections.reverseOrder());
		Instances ret = new Instances(m_Train, m_kNN);

		for (int o=0;o<m_kNN;o++) {
			ret.add(m_Train.instance(objectIndex.get(o).object));
			//System.out.println("val: " + objectIndex.get(o).sim);
		}
